...
function snabbdomBark (rootNode, pith) {
  return snabbdomInnerBark(rootNode,
    addPathRay([rootId++],
      assignPith((ths, rays) => {
        const path = rays.path.toString('.')
        return [{
          put: hf$ => ths.put(hf$.map(hf =>
            hf((s, d, c) => Object.assign(h(s, d, c), {path}))
          ))
        }, {
...
function addPathRay (path, pith) {
  return function addPathRayPith (rays) {
    var i = 0
    console.log(rays.path ? rays.path.join('.') : rays.path, path.join('.'), pith.toString())
    pith.call(Object.assign({}, this, {
      node: (x, pith) => this.node(x, addPathRay(path.concat(i++), pith))
    }), Object.assign({}, rays, { path }))
  }
}
